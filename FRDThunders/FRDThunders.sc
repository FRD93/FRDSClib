FRDThunders {
	var synth, reverb, rbus, bus, routine, win, play_b, trigger_b, hpf_s, rate_s, amp_s, r1_s, r2_s, r3_s, t1_s, t2_s, t3_s, dry1_s, dry2_s, dry3_s;
	var hpf, rate, amp, r1, r2, r3, t1, t2, t3, dry1, dry2, dry3;
	var is_playing, outCh;
	...
	*new {
	...
	hpf_ { | hpf_ |
		"Madonna puttana".postln;
		hpf = hpf_.clip(502, 15000);
		try{
			hpf_s.value_(Map(hpf, 502, 15000, 0.0, 1.0));
		};
	}
	...
	rate_ { | rate_ |
		rate = rate_.clip(0.01, 0.3);
		try{
			rate_s.value_(Map(rate, 0.01, 0.3, 0.0, 1.0));
		};
	}
		...
		if(is_playing.not, {
			...
			routine = Routine{
				loop{
					...
					(24..48).choose.do({ | id |
						Synth.before(trev, \ThunderSpike, [\amp,amp + rrand(amp.neg / 2, amp / 2), \out, bus, \hpf, hpf]);
						exprand(0.001, 0.03).wait;
					});
					trev.release(2);
	...
	startDSP {
		...
		reverb = Synth.head(Server.default, \ThunderRevLast, [\in, rbus, \out, outCh]);
	}
	...
	showGUI {
		...
		trigger_b  = Button().states_([["Trigger"]]).action_({
			this.trigger();
		});
