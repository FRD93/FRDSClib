FRDBackgroundNoiser {
	...
	*new { | inCh=20, outCh=0, wet=0.0, feed=0.35, clearness=0, centerfreq=2656, width=0.5, addAction='addToTail', actionNode=1 |
	...
	init { | inCh, outCh, wet, feed, clearness, centerfreq, width, addAction, actionNode |
		...
		this.computeGains();
		reverb = Synth(\FRDBackgroundNoiser, [\inCh, inCh_r, \outCh, outCh_r, \wet, wet_r, \feed, feed_r, \lowshelff, centerfreq_r, \hishelff, centerfreq_r, \lowshelfg, higain_r, \hishelf_g, lowgain_r], actionNode_r, addAction_r);
		hasGUI = false;
	...
	*/
	writeSynthDef {
		SynthDef(\FRDBackgroundNoiser, {| buf, outCh, rate=1, delT=0.25, delD=0.6, round=0.01, room=50, time=6, damp=0.5 |
			var player;
			player = PlayBuf.ar(2, buf, rate, loop: 1);
			player = AllpassC.ar(player, 5, delT, delD);
			player = player.round(round);
			player = GVerb.ar(player, room, time, damp, drylevel: 0);
			player = player - player.cubed;
			Out.ar(outCh, player);
		}).writeDefFile.add;
